...
class Croupier:
    ...
    def add_player(self, conn, addr, username):
        if(conn not in self.players.keys() and username not in self.players_conn.keys()):
            self.players[conn] = {
                "conn":conn,
                "address":addr,
                "username":username,
            ...
            croupier_logger.warning("Player already exists or username already taken")
            return 0
    ...
    def send_online_players(self, conn):
        ...
        croupier_logger.info("Sent list of online users to server")
        return payload
    ...
    def create_table(self, request, conn):
        if((request["table"] not in self.tables.keys()) and self.tables_number < 4):
            ...
            self.tables[request["table"]] = {
                ...
                "order":[request["username"]],
                "cards_dist_idx":0, # cards distribution index
                "players":{
                    request["username"]:{
    ...
    def shuffle_player_order(self, table, ntimes=1):
        for i in range(0, ntimes):
            random.shuffle(self.tables[table]["order"])

    def shuffle_cards(self, table, ntimes=1):
        print("table: " + str(table))
        for i in range(0, ntimes):
            random.shuffle(self.tables[table]["cards"])

    def give_shuffled_cards(self, table, conn):
        # shuffle the cards
        self.shuffle_cards(table)
        payload = {
            "operation":"croupier@give_shuffled_cards",
            "cards":self.tables[table]["cards"],
            "table":table
        }
        croupier_logger.info("Sent deck")
        return payload
    ...
    def join_player_table(self, request, conn):
        ...
        if(table in self.tables.keys() and self.tables[table]["nplayers"] < self.tables[table]["limit"]):
            if(username not in self.tables[table]["players"].keys()):
                # add player to the dict of players
                self.tables[table]["players"][username] = {
                    "username":username,
                    "points":0,
                    "hand":[]
                }
                # update the number of players in the table
                self.tables[table]["nplayers"] += 1
                # update the table's player order
                self.tables[table]["order"].append(username)
                # update player
                ...
                if(self.tables[table]["nplayers"] == self.tables[table]["limit"]):
                    ...
                    croupier_logger.info("Shuffling player order at table " + table)
                    self.shuffle_player_order(table)
                    # shuffle deck
                    croupier_logger.info("Shuffling cards at table " + table)
                    self.shuffle_cards(table)
                    # get conn of players in the table
                    for username in self.tables[table]["order"]:
